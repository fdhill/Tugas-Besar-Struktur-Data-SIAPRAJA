 ...
 * @author Pongo
 */
public class MonitoringGuruController {
    MonitoringGuruView view;
    DataSourceMonitoring datasource;
    
    int selectedTabelId = -1;


    public MonitoringGuruController(DataSourceMonitoring datasource, MonitoringGuruView view) {
        this.view = view;
        this.datasource = datasource;
        
        bassicLayout();
        
        datasource.refreshTableModelGuru(DataSource.getSession().getId_data());
        this.view.getjTable1().setModel(datasource.getTableModel());
        ((DataSourceMonitoring)this.datasource).refreshTableModelGuru(DataSource.getSession().getId_data());
        ...
        this.view.getjBtnPresensi().addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                new LihatPresensiGuru(new DataSourcePresensi(), new PresensiDanKegiatanForGuruView(), selectedTabelId).show();
